...
void MUSCLReconstructAll(const Grid& g, ReconstructedValues& RV){
    ...
    int j_index_final = g.ny;
    int i_index_final = g.nx;
    
    for (int j = -1; j <= j_index_final; j++){
        int j_padded = g.gj(j);                     // index accounting for ghost cells (otherwise -1 passed into indexC(), which crashes) 
        for (int i = -1; i <= i_index_final; i++){
            int i_padded = g.gi(i);
            int id = g.indexC(i_padded, j_padded);

            for (size_t a = 0; a < Fields.size(); a++){
            // Perform MUSCL on values in x and y separately
                FaceValues valuesX = MUSCLReconstructOne(Fields[a].field, i_padded, j_padded, Direction::X, g);
                Fields[a].L[id] = valuesX.L; Fields[a].R[id] = valuesX.R;
                
                FaceValues valuesY = MUSCLReconstructOne(Fields[a].field, i_padded, j_padded, Direction::Y, g);
            ...
            FaceValues bx_Y = MUSCLReconstructOne(g.bxc, i_padded, j_padded, Direction::Y, g);
            ...
            FaceValues by_X = MUSCLReconstructOne(g.byc, i_padded, j_padded, Direction::X, g);

...
void time_derivatives(const integrator_reserved_memory& memory,  const Grid& grid_div, global_state_dot& dU_explicit_rhs_memory) { // calculates the time derivatives of each quantity 
    ...
    const int j_index_initial = 0;
    const int j_index_final = total_y_nodes;
    const int i_index_initial = 0;
    const int i_index_final = total_x_nodes;
...
void generate_A_matrix(const Grid& grid_solve, integrator_reserved_memory& matrix_store, const double alpha) { // builds a matrix representing the discretised Laplacian operator in 2D, using a 5-point finite-difference stencil
    ...
    const int j_start = 0;
    const int j_end = y_size;
    const int i_start = 0;
    const int i_end = x_size;
...
void sausage_instability_initial_conditions(Grid& global_state) {
    ...
    constexpr double pi = 3.14159265359;
    const double domain_height = global_state.ny * global_state.cell_height;
    ...
    const double x_mid = 0.5 * global_state.nxt * global_state.cell_width;
...
void decaying_magnetic_sine_wave_initial_conditions(Grid& grid_dmw) {
    ...
    for (int j = 0; j < grid_y_node_num; j++) {
        for (int i = 0; i < grid_x_node_num; i++) {
            ...
            int idx = grid_dmw.indexC(i, j);

            double x_position = (i - ghost_node_num + 0.5) * cell_width;
            double y_position = (j - ghost_node_num + 0.5) * cell_height; 
            ...
            grid_dmw.rho[idx] = u.rho;
            grid_dmw.mx[idx]  = u.mx;
            grid_dmw.my[idx]  = u.my;
            grid_dmw.E[idx]   = u.E;
            grid_dmw.bxf[idx] = bx;
            grid_dmw.byf[idx] = by;
            grid_dmw.p[idx]   = p;
...
double CFL_condition_checker(const Grid& grid_checker) {
    ...
    double maximum_dt_resistive = 1e10; // default if resisitivit == 0, 
    ...
    for (int j = 0; j < total_y_nodes; j++) {
        for (int i = 0; i < total_x_nodes; i++) {
            ...
            if (current_fastest_speed > max_fluid_speed) {
                max_fluid_speed = current_fastest_speed;
            }
        }
    }
...
int main(){
    ...
    const double animation_duration = 10.0; // seconds
    const int snapshot_frequency = 5;     // increase this number to speed up animation, and vice versa
    const int x_node_count = 200;
    ...
    //decaying_magnetic_sine_wave_initial_conditions(global_state);
    ...
    double time_interval = CFL_condition_checker(global_state); // you can override to a constant value if you wish, however, the code may not run smoothly if so
    ...
    while (current_sim_time < animation_duration) {
        ...
        double current_time_step = (frame_index == 0) ? time_interval : CFL_condition_checker(global_state);

...
void get_electric_field_at_cell_corners(integrator_reserved_memory& B_memory, Grid& grid_B) { // function that calculates the explicit rhs terms of the time derivative of both B field components
    ...
    for (int j = j_index_start; j < j_index_end; j++){    
        for (int i = i_index_start; i < i_index_end; i++){
            ...
            // calculating the direction of the velocity at the current corner to utilise for Upwind Constrained Transport Method
            ...
            if (abs(vy_corner) < 1e-12) {Ez_y = 0.5 * (electric_field_south + electric_field_north);}
            else {
                Ez_y = (vy_corner > 0.0) ? electric_field_south : electric_field_north; // taking North direction as positive
            }

            if (abs(vx_corner) < 1e-12) {Ez_x = 0.5 * (electric_field_east + electric_field_west);}
            else {
...
int main(){
    ...
    double animation_duration = 3.5;   // seconds

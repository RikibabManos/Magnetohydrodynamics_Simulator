...
void magnetic_wave_test_initial_values(Grid& grid_mw) {
    ...
    const double kx = 2.0 * pi / width_of_cell;
    const double ky = 2.0 * pi / height_of_cell;
...
void gaussian_density_pulse_test_initial_values(Grid& grid_gauss) {
    ...
    const double pulse_radius = 10.0;
...
void acoustic_wave_test_initial_values(Grid& grid_ac) {
    ...
    const double amplitude_x = 0.1;
    const double amplitude_y = 0.1;
    const double bx  = 0.0;
    const double by  = 0.0;
...
void sausage_instability_initial_conditions(Grid& global_state) {
    ...
    constexpr double pi = 3.14159265359;
    const double domain_height = global_state.nyt * global_state.cell_height;
...
int main(){
    ...
    const int snapshot_frequency = 10;     // increase this number to speed up animation, and vice versa

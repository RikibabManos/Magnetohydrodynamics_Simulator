...
struct global_state_dot { // structure that holds all the derivative values of important variables, note that the 
    ...
    global_state_dot(int cell_total)
...
inline void get_intermediary_grid(vector<double>& grid, vector<double> list, double scalar){ // helper function to compute the new grid vector states 
